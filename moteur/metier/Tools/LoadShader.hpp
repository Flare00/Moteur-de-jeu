...
GLuint LoadShaders(std::string vertexPath, std::string fragmentPath, std::string geometryPath = "", std::string tesselationControlPath = "", std::string tesselationEvalPath = "")
{
	...
	if (vertexData.length() > 0)
	{
		...
		if (fragmentData.length() > 0)
		{
			...
			if (CompileShader(VertexShaderID, vertexData) == GL_FALSE)
				return 0;
			printf("Compiling shader : %s\n", fragmentPath.c_str());
			if (CompileShader(FragmentShaderID, fragmentData) == GL_FALSE)
				return 0;
			...
			if (geometryPath.length() > 0)
			{
				...
				if (geometryData.length() > 0)
				{
					...
					printf("Compiling shader : %s\n", geometryPath.c_str());
					if (CompileShader(GeometryShaderID, geometryData) == GL_FALSE)
						return 0;
				}
				else
				{
			...
			if (tesselationControlPath.length() > 0)
			{
				...
				if (tesselationControlData.length() > 0)
				{
					...
					if (CompileShader(TesselationControlShaderID, tesselationControlData) == GL_FALSE)
						return 0;
				}
				else
				{
			...
			if (tesselationEvalPath.length() > 0)
			{
				...
				if (tesselationEvalData.length() > 0)
				{
					...
					if (CompileShader(TesselationEvalShaderID, tesselationEvalData) == GL_FALSE)
						return 0;
				}
				else
				{
