...
public:
	...
	/// </summary>
	DirectionnalLight(vec3 position, vec3 direction, vec3 color = vec3(1), float intensity = 1.0f, float distance = 100.0f, int shadowResolution = 8, bool generateShadow = true) : ILight(Type::DIRECTIONAL, position, color, intensity), shadow(shadowResolution) {
